...
import com.bugielmarek.timetable.controllers.ContactController;
...
public class ContactControllerTest {
	...
	public void testFindContact_MatchingResults_PageIsNotDefault() throws Exception {
		...
		when(mockService.searchInputHasResult(formClass, 2)).thenReturn(true);
		...
		mockMvc.perform(post("/contacts/findcontact?p=2")
				...
				.andExpect(flash().attributeCount(1))
				;
		verify(mockService, times(1)).searchInputHasResult(formClass, 2);
	...
	public void testFindContact_NoResultsMatching() throws Exception {
		...
		mockMvc.perform(post("/contacts/findcontact")
				...
				.andExpect(model().attributeHasFieldErrors("formClass", "contact.name"))
				;
		verify(mockService, times(1)).searchInputHasResult(formClass, 1);
		verifyNoMoreInteractions(mockService);
	}
